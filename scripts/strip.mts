...
try {
	...
	for await (const filePath of glob(includeGlobs, { cwd, exclude: excludeGlobs })) {
		...
		if (extension === '.ts') {
			...
			// FIXME: this is not an acceptable solution to rewriting module paths.
			// Amaro doesn't do this, this might be a full blocker to use using it
			const codeWithModulePaths = code
				.replaceAll(/from(\s+)'([^']+)\.ts'/g, "from$1'$2.js'")
				.replaceAll(/import\('([^']+)\.ts'\)/g, "import('$1.js')");

			await writeFile(outputPath, codeWithModulePaths);
		}
	}
} catch (error) {
