...
describe('@dotcom-reliability-kit/errors/lib/base-error', () => {
	...
	describe('new BaseError(data)', () => {
		...
		it('normalizes the passed in error code', () => {
			assert.strictEqual(BaseError.normalizeErrorCode.mock.calls.length, 1);
			assert.deepEqual(BaseError.normalizeErrorCode.mock.calls[0].arguments, ['mock_code']);
		});
		...
		describe('.data', () => {
			it('is set to an object containing the extra keys in `data`', () => {
				assert.deepEqual(instance.data, { extra: 'mock extra data' });
			});
		});
